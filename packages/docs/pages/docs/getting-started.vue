<template>
  <div class="doc-container relative">
    ...
    <div class="relative z-10">
      <h1 class="doc-heading-1 shimmer-effect">Getting Started with jsonreader</h1>
      ...
      <div class="bg-accent/10 p-4 mb-8 rounded-lg border border-accent/30 card-noise fade-in">
        <div class="flex items-start">
          ...
          <div>
            <p class="text-black">jsonreader is a specialized utility for processing JSON data from streams in real-time, with a particular focus on handling responses from AI tool calls efficiently.</p>
          </div>
        </div>
      </div>
      ...
      <p class="doc-text fade-in-delayed">Add jsonreader to your project using your preferred package manager:</p>
...
pnpm add jsonreader</pre>
      ...
      <h2 class="doc-heading-2 fade-in-delayed" style="animation-delay: 0.3s;">Why Use jsonreader?</h2>

      <p class="doc-text fade-in-delayed" style="animation-delay: 0.4s;">When working with AI tool calls or any large JSON responses, jsonreader provides several key advantages:</p>

      <ol class="doc-list fade-in-delayed" style="animation-delay: 0.5s;">
        <li class="doc-list-item"><span class="accent-text">Process data as it arrives</span>: Don't wait for the entire API response to complete.</li>
        <li class="doc-list-item"><span class="accent-text">Early UI updates</span>: Show users results immediately as they become available.</li>
        <li class="doc-list-item"><span class="accent-text">Path-specific extraction</span>: Access specific parts of the JSON as soon as they appear in the stream.</li>
        <li class="doc-list-item"><span class="accent-text">Optimized for AI tools</span>: Perfect for processing responses from AI function calls that can be large and slow to complete.</li>
      </ol>

      <h2 class="doc-heading-2 fade-in-delayed" style="animation-delay: 0.6s;">Basic Usage</h2>

      <h3 class="doc-heading-3 fade-in-delayed" style="animation-delay: 0.7s;">Streaming JSON Processing</h3>

      <p class="doc-text fade-in-delayed" style="animation-delay: 0.8s;">The simplest way to use jsonreader is to process JSON data as it streams in:</p>
...
<span class="text-accent">async</span> <span class="text-accent">function</span> <span class="text-white">processStreamingData</span>() {
  ...
  <span class="text-accent">for</span> <span class="text-accent">await</span> (<span class="text-accent">const</span> partialData <span class="text-accent">of</span> jsonReader(reader)) {
    ...
    updateUI(partialData);
      ...
      <p class="doc-text">In this example, <code class="inline-code">partialData</code> will contain progressively more complete JSON objects as the data streams in. Each iteration yields the most complete JSON structure possible with the data received so far.</p>

      <h3 class="doc-heading-3">Extracting Specific Paths</h3>

      <p class="doc-text">You can also extract specific paths from the JSON as they become available:</p>
...
<span class="text-accent">async</span> <span class="text-accent">function</span> <span class="text-white">processToolResults</span>() {
  ...
  <span class="text-accent">for</span> <span class="text-accent">await</span> (<span class="text-accent">const</span> { results } <span class="text-accent">of</span> jsonReader(reader, {
    ...
    <span class="text-accent">if</span> (results?.length) {
      ...
      <p class="doc-text">This approach is particularly useful when working with AI tool calls that may return large arrays of results that you want to display incrementally as they become available.</p>

      <h2 class="doc-heading-2">Using jsonPathReader</h2>

      <p class="doc-text">For more targeted extraction of specific JSON paths, you can use the <code class="inline-code">jsonPathReader</code> function:</p>
...
<span class="text-accent">for</span> <span class="text-accent">await</span> (<span class="text-accent">const</span> [value, path] <span class="text-accent">of</span> jsonPathReader(reader, paths)) {
  <span class="text-accent">if</span> (path === <span class="text-white">'results'</span>) {
    ...
    trackItem(value);
      ...
      <p class="doc-text">The <code class="inline-code">jsonPathReader</code> approach is ideal when you need to handle updates to multiple specific paths independently. It yields each value and path as a tuple ([value, path]) as soon as they change in the stream. You can use the <code class="inline-code">'*'</code> wildcard character in path segments to match any property name or array index at that position.</p>
...
<span class="text-accent">try</span> {
  ...
  console.error(<span class="text-white">'Error processing JSON:'</span>, error);
      ...
      <div class="bg-accent/10 p-4 mt-8 mb-8 rounded-lg border border-accent/30 card-noise">
        <h3 class="text-lg font-bold mb-2">Next Steps</h3>
        <ul class="space-y-1">
          <li class="transition-transform hover:translate-x-2 duration-300"><a href="/docs/api-reference" class="text-accent hover:underline flex items-center"><span class="mr-2">→</span> API Reference</a> - Explore the complete API documentation</li>
          <li class="transition-transform hover:translate-x-2 duration-300"><a href="/docs/examples" class="text-accent hover:underline flex items-center"><span class="mr-2">→</span> Examples</a> - See more detailed examples</li>
        </ul>
      </div>
    </div>
